...
import { createHersteller, FormState } from '@/app/dashboard/admin/users/actions/herstellerActions';
import { SubmitButton } from '@/components/buttons/submit-button';

const initialState: FormState = { message: '', errors: {} };

export function CreateHerstellerForm() {
    const [state, dispatch] = useActionState(createHersteller, initialState);

  return (
    <form action={dispatch} className="space-y-8 bg-white p-8 rounded-lg shadow-md">
      <div className="border-b border-gray-900/10 pb-8">
        <h2 className="text-xl font-semibold leading-7 text-gray-900">Hersteller anlegen</h2>
        <p className="mt-1 text-sm leading-6 text-gray-600">
          Füllen Sie die Details aus, um einen neuen Benutzer mit der Rolle 'Hersteller' zu erstellen.
        </p>
      </div>
