...
export type HaendlerActionState = {
  message: string;
...
// --- SERVER ACTION: createHaendler ---
export async function createHaendler(
  previousState: HaendlerActionState,
  formData: FormData
): Promise<HaendlerActionState> {
  const validation = HaendlerFormSchema.safeParse(Object.fromEntries(formData.entries()));
  ...
  try {
    // Step 2: The trigger `create_user_profile_and_role` handles profile and role table insertion.

    // Step 3: Update the 'haendler' table with additional info
    const { error: haendlerError } = await supabaseAdmin
      .from('haendler')
      .update({
        firma: company_name,
        webseite: website,
    ...
    if (haendlerError) {
      console.error('Händler Update Error:', haendlerError.message);
      throw new Error('Der Benutzer wurde erstellt, aber die Händler-Profilinformationen konnten nicht gespeichert werden.');
    }

  } catch (error: unknown) {
    // If any step after user creation fails, delete the auth user to avoid orphans.
